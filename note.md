...
<blockquote>이 논문에서는 모바일 폰 사용자의 통화 데이터를 분석하여, 인간의 이동성과 행동 패턴에 대한 다양한 특징을 추출하였습니다. 이를 통해, 자주 방문하는 장소가 주거지와 직장이라는 것을 확인하였으며, 이러한 장소 이외에는 다양한 장소를 방문하는 경우가 많았습니다. 또한, 이동 거리와 통화 시간 간의 상관 관계를 분석한 결과, 이동 거리가 긴 경우 통화 시간도 길어지는 경향이 있었습니다. 이러한 분석 결과를 통해, 교통체증 및 도시 계획 등과 같은 분야에서 인간의 이동성과 행동 패턴을 더 잘 이해하고, 이를 활용하여 보다 효율적인 정책을 수립할 수 있을 것으로 기대됩니다.</blockquote>
<h4><li>Dataset</li></h4>
논문에 사용한 데이터는 포르투갈 10만 명의 모바일 폰 사용자의 통화 데이터입니다. 통화시작과 종료때의 안테나를 기준으로 위치와 거리를 얻었습니다. 데이터를 전처리할 때, 통화수가 하루에 평균 1개이상이고, 80%이상 연달아 통화하는 유저만을 선별하여 진행하였습니다. 
<img src="source/image05.png">
...
<h4><li>Clustering result</li></h4>
k=3으로 클러스터링하였습니다. (주거지, 직장, 그외)
<img src="source/image04.png">
한 지역에 위치한 경우가 대부분이였고, 하루에 통화를 한 주요지역이 4곳이하인 경우가 95%였습니다. 주요지역이 한곳인 경우가 많았는데, 해당 논문에서는 전화를 일로 쓰는 용도와 사적으로 쓰는 용도로 나눠서 쓰는 사람이 많기 때문이라 추측하였습니다.
